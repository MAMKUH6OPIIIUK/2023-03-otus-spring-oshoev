...
public class TestingDaoCsv implements TestingDao {
     ...
     */
    public TestingDaoCsv(@Value("${testing.dao.csv}") String csvResourceName) {
     ...
     */
    private static void readFromInputStream(InputStream inputStream, List<Testing> testings)
            throws IOException {
     ...
     *                                     не является числом
     */
    private static void parseLineAndSave(String[] line, List<Testing> testings) {
        if (line.length != 5) {
            throw new IncorrectCsvFormatException("Q&A CSV resource must contain 5 columns");
        }
        Testing savedTesting = getTestingByTheme(testings, line[0]);
        if (savedTesting == null) {
            savedTesting = saveNewTesting(testings, line[0]);
        }
        QuestionType rowQuestionType;
        try {
            rowQuestionType = QuestionType.fromString(line[1]);
        } catch (IllegalArgumentException e) {
            throw new IncorrectCsvFormatException("Column 1: " + e.getMessage());
        }
        Question existentQuestion = savedTesting.getQuestionByTypeAndText(rowQuestionType, line[2]);
        if (existentQuestion != null) {
            updateSavedQuestion(existentQuestion, line);
        } else {
            saveNewQuestion(savedTesting, line);
        }
    }
     ...
     */
    private static Testing getTestingByTheme(List<Testing> testings, String themeName) {
     ...
     */
    private static Testing saveNewTesting(List<Testing> testings, String themeName) {
     ...
     */
    private static void saveNewQuestion(Testing testing, String[] line) {
        try {
     ...
     * @throws IncorrectCsvFormatException См. {@link #parseLineAndSave(String[], List)}
     */
    private static void updateSavedQuestion(Question savedQuestion, String[] line) {
